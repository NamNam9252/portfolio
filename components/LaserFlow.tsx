...
// Global timing state to persist across navigations
let gFlowTime = 100;
let gFogTime = 100;
...
export const LaserFlow: React.FC<Props> = ({
  ...
  const hexToRGB = (hex: string) => {
    let c = (hex || '').trim();
    if (!c) return { r: 1, g: 1, b: 1 }; // Default to white if empty
    if (c.length === 3)
      c = c
        .split('')
        .map(x => x + x)
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("LaserFlow: Failed to create WebGLRenderer", e);
      return;
    }
    ...
    const uniforms = {
      ...
      uFlowTime: { value: gFlowTime },
      uFogTime: { value: gFogTime },
    ...
    let prevTime = 0;
    // Fade logic removed for instant start
    ...
    const animate = () => {
      ...
      gFlowTime = uniforms.uFlowTime.value;
      gFogTime = uniforms.uFogTime.value;

      // Fade logic removed to Force instant start
